...
from collections import defaultdict #νεο 
...
class CSP(search.Problem):
    ...
    def __init__(self, variables, domains, neighbors, constraints):
        ...
        # Υπολογισμός βαρών για όλα τα ζεύγη
        for var in self.variables:
            for varx in self.variables:
                if var != varx:
                    # Δημιουργία ζεύγους 
                    pair = (var, varx) if var < varx else (varx, var)
                    # Αν δεν έχει καταχωρηθεί
...
def revise(csp, Xi, Xj, removals, checks=0):
    ...
    for x in csp.curr_domains[Xi][:]:
        ...
        if conflict:
            ...
            csp.weight[(Xi, Xj)]+=1 #new code προσθετουμε βαρος στο ζευγαρι
            revised = True
...
def dom_wdeg(assignment, csp):
    """
    Υλοποίηση της ευρετικής dom/wdeg:
    - Επιλέγει τη μεταβλητή που ελαχιστοποιεί τον λόγο: μέγεθος domain / άθροισμα βαρών.
    """
    min_ratio = float('inf')  # Αρχικοποίηση με άπειρο για εύρεση ελάχιστης τιμής
    weights_sum = defaultdict(int)  # Χρήση defaultdict για αυτόματη αρχικοποίηση σε 0
    selected_var = None

    # Αν δεν υπάρχουν προκαθορισμένα domains, fallback στην `first_unassigned_variable`
    if csp.curr_domains is None:
        return first_unassigned_variable(assignment, csp)

    for var in csp.variables:
        # Παράλειψε μεταβλητές που έχουν ήδη τιμή στο assignment
        if var in assignment:
            continue

        # Υπολόγισε το άθροισμα των βαρών για τους μη ανατεθειμένους γείτονες
        for neighbor in csp.neighbors[var]:
            if neighbor not in assignment:
                weights_sum[var] += csp.weight.get((var, neighbor), 0)

        # Υπολόγισε τον λόγο: μέγεθος domain / άθροισμα βαρών
        if weights_sum[var] > 0:  # Αποφυγή διαίρεσης με το 0
            domain_size = len(csp.curr_domains[var])
            ratio = domain_size / weights_sum[var]

            # Ενημέρωση της μεταβλητής με τον μικρότερο λόγο
            if ratio < min_ratio:
                min_ratio = ratio
                selected_var = var

    # Επιστροφή της καλύτερης μεταβλητής, ή fallback αν καμία δεν βρεθεί
    return selected_var if selected_var else first_unassigned_variable(assignment, csp)
